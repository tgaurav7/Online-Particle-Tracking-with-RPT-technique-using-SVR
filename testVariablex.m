...
[y_hat] = svmpredict(tst_data.y, tst_data.X, modelx);
%modelx
%size(y_hat)
%length(tst_data.y)
...
legend('X(mm) vs Readings(0.02s)');
y1 = max([trn_data.y; tst_data.y]);
y2 = min([trn_data.y; tst_data.y]);
axis([1 500 y2 y1]);
%filename = 'exp_results_x.xls';

fileID = fopen('exp_results_x.txt','a');
fprintf(fileID, '%f\n', xval);
fclose(fileID);
