...
[y_hat] = svmpredict(tst_data.y, tst_data.X, modelz);
%model
%size(y_hat)
%length(tst_data.y)
...
legend('Z(mm) vs Readings(0.02s)');
y1 = max([trn_data.y; tst_data.y]);
y2 = min([trn_data.y; tst_data.y]);
axis([1 500 y2 y1]);
%filename = 'exp_results_z.xls';

fileID = fopen('exp_results_z.txt','a');
fprintf(fileID, '%f\n', zval);
fclose(fileID);
%xlRange = 'A1';
%sheet = num;
